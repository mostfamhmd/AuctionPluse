...
import 'package:smart_auction/feature/Home/presentation/view/widgets/live_auction_now.dart';
import 'package:smart_auction/feature/Home/presentation/view/widgets/no_live_auction_now.dart';
...
class RealTimeLiveAuction extends StatelessWidget {
  const RealTimeLiveAuction({super.key});
        ...
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          try {
            ...
            for (int i = 0; i < snapshot.data!.rooms!.length; i++) {
              if (snapshot.data!.rooms![i].event! == true &&
                  snapshot.data!.rooms![i].ended! == false) {
                scheduleLive.add(snapshot.data!.rooms![i]);
              }
            }
            return scheduleLive.isEmpty
                ? const NoLiveAuctionNow()
                : LiveAuctionNow(scheduleLive: scheduleLive);
